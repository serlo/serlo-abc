...
const raw_words = require('./words-data.js');
const words = {};
...
raw_words.forEach((raw_word, index) => {
  ...
  "${id}": {
    ...
    image: ${image && `require('./images/${image}') as IImageAsset`},
    sound: ${sound && `require('./sounds/${sound}') as ISoundAsset`},
    longSound: ${longSound && `require('./sounds/${longSound}') as ISoundAsset`}
  },
  `;
});
...
fs.writeFileSync(path.join(assetsPath, 'words.ts'), str);
